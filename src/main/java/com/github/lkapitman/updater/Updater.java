...
import com.github.lkapitman.updater.json.DataFile;
import com.github.lkapitman.utils.Constants;
import com.google.gson.Gson;
import com.github.lkapitman.updater.json.Data;
import com.github.lkapitman.utils.Utils;
...
public class Updater {
    ...
    public void start(){
        if (!dir.exists()){
            dir.mkdirs();
            Constants.LOGGER.log("Folder " + dir.getName() + " doesn't exist, mkdirs folder.");
        }else{
            Constants.LOGGER.log("Folder " + dir.getName() + " exist, start check url.");
        }
    ...
    private void jsonDeserialize(String str){
        ...
        if(data.getMaintenance().isMaintenance()){
            System.out.println("не могу запустить у меня есть бассейн\n");
            System.exit(0);
        }
        ...
        for (DataFile file : data.getFiles()){
            ...
            try {
                if(!Utils.getMD5(client_file).equals(file.getHash())){
                    Constants.LOGGER.log("File " + client_file.getName() + " has a bad hash '" + Utils.getMD5(client_file) + "' hash on server => " + file.getHash());
                    this.job.addDownloadable(client_file, file.getHash(), file.getUrl());
                    this.ignoredFiles.add(client_file.getAbsolutePath());
                }else{
                    this.ignoredFiles.add(client_file.getAbsolutePath());
